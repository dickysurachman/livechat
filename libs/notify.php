...
function log_notification($locale, $kind, $to, $subj, $text, $refop, $link,$email)
{
	global $mysqlprefix;
	$query = sprintf(
		"insert into ${mysqlprefix}chatnotification (locale,vckind,vcto,vcsubject,tmessage,refoperator,dtmcreated,email) values ('%s','%s','%s','%s','%s',%s,%s,'%s')",
...
function mibew_mail($toaddr, $reply_to, $subject, $body, $link)
{
	...
	log_notification($current_locale, "mail", $toaddr, $subject, $body, null, $link,$reply_to);
